...
void processKey(const char* key) {
  ...
  if (programMode) {
    ...
    // Temporary replacement for the physical W/PRGM-RUN switch.
    if (prefixH && strcmp(key, "R/S") == 0) {
      prefixH = false;
      programMode = false;
      programRunning = false;
      progCursor = programSize;
      Serial.println("Leaving PRGM mode");
      showStack();
      printProgramPosition("RUN mode");
      return;
    }
  ...
  if (prefixH) {
    if (strcmp(key, "R/S") == 0)      { prefixH = false; programMode = true; programRunning = false; progCursor = programSize; Serial.println("Entering PRGM mode"); printProgramPosition("PRGM mode"); return; }
  ...
  if (strcmp(key, "A") == 0 || strcmp(key, "B") == 0 ||
    ...
    // Original HP-67 behavior: once any user program exists, A-E stop being
    // the default math shortcuts and become program/routine selectors.
    if (programSize > 0) {
      startProgramAtLabel(key[0]);
      return;
    }
...
void setup() {
  ...
  Serial.println("  h R/S  = toggle PRGM mode (temporary replacement for W/PRGM-RUN switch)");
  ...
  Serial.println("  RUN A-E = run matching labeled routine; R/S = run/stop; SST = single-step");
}

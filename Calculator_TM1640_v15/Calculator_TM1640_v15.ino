...
void processKey(const char* key) {
  ...
  if (strcmp(key, "A") == 0 || strcmp(key, "B") == 0 ||
    ...
    // In RUN mode these five physical keys always perform the functions
    // printed above them on this retrofit. Program-mode label entry still
    // accepts A-E earlier in processKey().
...
void setup() {
  ...
  Serial.println("*** V15 STACK / A-E / PROGRAM SWITCH FIX ***");
  Serial.println();
  ...
  Serial.println("  RUN A-E = 1/x, sqrt(x), y^x, R-down, X<>Y");
}
